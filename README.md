...
- **IR Point Tracker (2017)** - I wanted to be able to walk around my house with my eyes closed. My solution was to set up a camera at a known physical position/angle that would track an IR LED on a hat. To avoid collision with walls and furniture, a tone would play in my headphones to indicate whether I need to turn left or right. I used OpenCV to find the 2D screen position of the LED, and then converted it into a physically accurate position in 3D space using raycasting. Built with Java, OpenCV, and one of those cheap Wyze cams.
